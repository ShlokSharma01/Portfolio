...
/* ── Card hover: enhanced bloom + image zoom ─────────────────────────── */
function useCardHover(glowIntensity = 0.26) {
  const cardRef  = useRef<HTMLDivElement>(null);
  const imgRef   = useRef<HTMLImageElement>(null);
  const glowRef  = useRef<HTMLDivElement>(null); // radial bloom overlay

  const enter = () => {
    if (cardRef.current) {
      cardRef.current.style.transform = 'translateY(-8px)';
      cardRef.current.style.boxShadow =
        `0 28px 80px rgba(225,14,31,${glowIntensity}), 0 0 0 1px rgba(225,14,31,0.38)`;
      cardRef.current.style.borderColor = 'rgba(225,14,31,0.5)';
    }
  ...
  return { cardRef, imgRef, glowRef, enter, leave };
}

/* ── Feature card ────────────────────────────────────────────────────── */
function FeatureCard({ project }: { project: Project }) {
  const { cardRef, imgRef, glowRef, enter, leave } = useCardHover(0.32);
  ...
  return (
    ...
    >
      {/* Radial bloom that fades in on hover */}
      <div
        ref={glowRef}
        aria-hidden="true"
        ...
        style={{
          background:
            'radial-gradient(ellipse 60% 70% at 30% 50%, rgba(225,14,31,0.1) 0%, transparent 70%)',
          opacity: 0,
      ...
      <div className="grid md:grid-cols-2" style={{ position: 'relative', zIndex: 2 }}>
        {/* Image */}
        <div className="relative overflow-hidden" style={{ minHeight: '280px' }}>
          ...
          <div className="absolute inset-0 pointer-events-none"
            style={{ background: 'linear-gradient(to right, transparent 60%, var(--card-bg))' }} />
          <div className="absolute inset-0 pointer-events-none"
            style={{ background: 'linear-gradient(to top, rgba(11,6,8,0.6) 0%, transparent 40%)' }} />
          {project.live && (
            <span className="absolute top-4 left-4 font-mono text-[0.58rem] tracking-[0.2em] uppercase px-2.5 py-1 rounded-full"
        ...
        <div className="relative p-8 md:p-10 flex flex-col justify-between" style={{ minHeight: '280px' }}>
          {/* Ghost index — data-ghost-num for parallax */}
          <div
            data-ghost-num
            aria-hidden="true"
            className="absolute font-display font-bold select-none pointer-events-none"
            style={{ fontSize: '9rem', lineHeight: 1, letterSpacing: '-0.04em', color: 'var(--text)', opacity: 0.055, top: '-1rem', right: '1rem', willChange: 'transform' }}
          >
            01
          </div>
...
/* ── Regular card (3-D tilt + enhanced hover bloom) ─────────────────── */
function RegularCard({ project, cardIndex }: { project: Project; cardIndex: number }) {
  const tiltRef = useRef<HTMLDivElement>(null);
  const { cardRef, imgRef, glowRef, enter, leave } = useCardHover();
      ...
      >
        ...
        >
          ...
          <div
            ...
            style={{
              background:
                'radial-gradient(ellipse 70% 60% at 50% 40%, rgba(225,14,31,0.1) 0%, transparent 70%)',
              opacity: 0,
              transition: 'opacity 0.4s ease',
              zIndex: 1,
            }}
          ...
          {/* Image */}
          <div className="relative overflow-hidden" style={{ aspectRatio: '16/9', zIndex: 2 }}>
            <img
              ref={imgRef}
              src={project.image}
              alt={project.title}
          ...
          {/* Info */}
          <div className="flex flex-col flex-1 p-6 relative" style={{ zIndex: 2 }}>
            {/* Ghost card index — data-ghost-num for parallax */}
            <div
              data-ghost-num
              aria-hidden="true"
              className="absolute font-display font-bold select-none pointer-events-none"
              style={{ fontSize: '5.5rem', lineHeight: 1, letterSpacing: '-0.04em', color: 'var(--text)', opacity: 0.055, top: '-0.5rem', right: '0.75rem', willChange: 'transform' }}
            >
              {label}
            </div>
...
export default function Projects() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      /* Ghost numbers parallax — scrub slower than cards, feel further back */
      if (!reducedMotion) {
        gsap.fromTo(
          '[data-ghost-num]',
          { y: 0 },
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ zIndex: 0 }}
      >
        {/* Blob A — crimson bloom bottom-right */}
        <div className="mesh-blob absolute" style={{
          width: '65vw', height: '65vw', borderRadius: '50%',
          right: '-15%', bottom: '-10%',
          background: 'radial-gradient(ellipse 50% 50% at 50% 50%, rgba(225,14,31,0.11) 0%, rgba(122,10,18,0.05) 45%, transparent 70%)',
          animation: 'mesh-a 28s ease-in-out infinite', willChange: 'transform',
        }} />
        {/* Blob B — deep left anchor */}
        <div className="mesh-blob absolute" style={{
          width: '50vw', height: '50vw', borderRadius: '50%',
          left: '-10%', top: '20%',
          background: 'radial-gradient(ellipse 50% 50% at 50% 50%, rgba(122,10,18,0.1) 0%, transparent 65%)',
          animation: 'mesh-b 34s ease-in-out infinite reverse', willChange: 'transform',
        }} />
        {/* Blob C — gold shimmer centre */}
        <div className="mesh-blob absolute" style={{
          width: '35vw', height: '35vw', borderRadius: '50%',
          left: '30%', top: '-5%',
          background: 'radial-gradient(ellipse 50% 50% at 50% 50%, rgba(212,175,55,0.05) 0%, transparent 60%)',
          animation: 'mesh-c 44s ease-in-out infinite', willChange: 'transform',
        }} />
      </div>

      {/* Ghost section number — also gets parallax via data-ghost-num */}
      <div
        data-ghost-num
        aria-hidden="true"
        className="absolute font-display font-bold select-none pointer-events-none"
